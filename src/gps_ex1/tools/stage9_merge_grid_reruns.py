"""Safely merge Stage 9 fixed-grid context reruns.

A rerun can replace a first-pass row only if:
- the frame was approved for that grid cell in stage9_grid_context_rerun_plan.csv;
- the rerun itself is accepted by visual/geometric verification;
- the rerun confidence improves enough over the base row.
"""
...
_REGION_RE = re.compile(r"grid_region_(\d+)")
...
def _read_plan(path: Path) -> dict[int, set[int]]:
    ...
    with path.open("r", newline="", encoding="utf-8") as fp:
        ...
        for row in reader:
            ...
            regions: set[int] = set()
            for part in (row.get("candidate_grid_regions") or "").replace(",", ";").split(";"):
                token = part.strip()
                if token:
...
def _region_from_path(path: Path) -> int:
    m = _REGION_RE.search(path.name)
    if m is None:
        raise ValueError(f"Could not parse grid region id from rerun filename: {path.name}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Safely merge Stage 9 fixed-grid reruns.")
    ...
    parser.add_argument("--rerun-glob", default="stage9_rerun_grid_region_*_top8.csv")
    ...
    rerun_paths = sorted(args.rerun_dir.glob(args.rerun_glob))
    if not rerun_paths:
        raise SystemExit(f"No rerun CSV files matched {args.rerun_glob} in {args.rerun_dir}")

    for rerun_path in rerun_paths:
        region_id = _region_from_path(rerun_path)
    ...
    extra_cols = [
        "stage9_grid_rerun_replaced",
        "stage9_grid_rerun_region_id",
        "stage9_grid_rerun_score",
        "stage9_grid_before_score",
        "stage9_grid_rerun_reason",
    ]
    ...
    with args.out.open("w", newline="", encoding="utf-8") as fp:
        ...
        for base in base_rows:
            ...
            if frame in replacements:
                ...
                output["stage9_grid_rerun_replaced"] = "1"
                output["stage9_grid_rerun_region_id"] = str(region_id)
                output["stage9_grid_rerun_score"] = f"{rerun_score:.6f}"
                output["stage9_grid_before_score"] = f"{base_score:.6f}"
                output["stage9_grid_rerun_reason"] = rerun.get("filter_reason", "")
            else:
                output["stage9_grid_rerun_replaced"] = "0"
                output["stage9_grid_rerun_region_id"] = ""
                output["stage9_grid_rerun_score"] = ""
                output["stage9_grid_before_score"] = ""
                output["stage9_grid_rerun_reason"] = ""
            writer.writerow({k: output.get(k, "") for k in out_header})

    print(f"Stage 9 read {len(rerun_paths)} rerun CSV files")
    print(f"Stage 9 merged {len(replacements)} fixed-grid context replacements into: {args.out}")
